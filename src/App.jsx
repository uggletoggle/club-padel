...
import {
  ...
  List, CalendarDays, Search, SearchCheck
} from 'lucide-react';
...
const ReservationModal = ({ court, isOpen, onClose, onSave, existingReservations, onDeleteRes, showToast, initialValues }) => {
  ...
  // Inicializar con valores pasados (desde el buscador) o por defecto (ahora)
...
// ... (Rest of simple components: Handle, GenericZone, etc.) ...
// INCLUIR AQUÍ LOS COMPONENTES DE SIEMPRE PARA QUE EL COPY-PASTE FUNCIONE
...
const PadelCourt = ({
  ...
  containerRef, mode, activeReservation, onManage
}) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ scale: 1.02, cursor: mode === 'design' ? 'grab' : 'pointer' }}
      whileDrag={{ scale: 1.05, cursor: 'grabbing', zIndex: 100 }}
      onDragEnd={(e, info) => {
        if (mode === 'design') onUpdate(data.id, { x: data.x + info.offset.x, y: data.y + info.offset.y });
      }}
      onClick={(e) => {
        e.stopPropagation();
        if (mode === 'design') onSelect(data.id);
        else onManage(data);
      }}
...
const GenericZone = ({ data, isSelected, onSelect, onUpdate, onRotate, onDelete, containerRef, mode }) => {
  ...
  return (
    ...
    >
      <div className={`w-full h-full relative transition-all rounded-md ${isSelected && mode === 'design' ? 'ring-2 ring-blue-500' : ''}`}>
        <div
          className={`w-full h-full bg-slate-700/80 backdrop-blur-sm border-2 border-slate-500 border-dashed flex items-center justify-center overflow-hidden ${mode === 'design' ? 'cursor-grab' : 'cursor-default'}`}
          onPointerDown={(e) => {
            if (mode === 'design') {
              onSelect(data.id);
...
export default function PadelClubPlanner() {
  ...
  // Managing Court now holds: { court, initialValues: { date, time, duration } } or just court
  ...
  return (
    <div className="w-full h-screen bg-slate-950 overflow-hidden flex flex-col font-sans select-none text-slate-200">
      ...
      <header className="h-16 border-b border-slate-800 bg-slate-900 flex items-center justify-between px-6 shadow-md z-40 relative">
        ...
        <div className="bg-slate-950 p-1 rounded-xl border border-slate-800 flex gap-1">
          <button
            ...
            className={`px-3 md:px-4 py-2 rounded-lg text-xs md:text-sm font-medium flex items-center gap-2 transition-all ${mode === 'design' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}
          >
            <LayoutTemplate size={16} /> <span className="hidden md:inline">Arquitecto</span>
          </button>
          <button
            onClick={() => setMode('manage')}
            className={`px-3 md:px-4 py-2 rounded-lg text-xs md:text-sm font-medium flex items-center gap-2 transition-all ${mode === 'manage' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}
          >
            <Armchair size={16} /> <span className="hidden md:inline">Gestión</span>
          </button>
          <button
            onClick={() => setMode('search')}
            className={`px-3 md:px-4 py-2 rounded-lg text-xs md:text-sm font-medium flex items-center gap-2 transition-all ${mode === 'search' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}
          >
            <Search size={16} /> <span className="hidden md:inline">Disponibilidad</span>
          </button>
        </div>
      </header>
      ...
      <div className="flex-1 relative bg-slate-950 overflow-hidden" onClick={() => setSelectedId(null)}>
        {mode === 'search' ? (
          <AvailabilityView
            ...
            onReserve={(court, values) => setManagingState({ court, initialValues: values })}
          />
        ) : (
          <>
      ...
      {mode === 'design' && (
        <DesignToolbar onAddCourt={addCourt} onAddZone={addZone} onClear={() => setElements([])} />
      )}

      {mode === 'manage' && (
        <TimeController currentDate={viewDate} onChange={setViewDate} />
      )}
